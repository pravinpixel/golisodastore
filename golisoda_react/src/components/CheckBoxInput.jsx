...
function CheckBoxInput({data, name}) {
  ...
  const handler = () => {
    ...
    var checkboxes = document.querySelectorAll(
      `.${name}-product-check-input:checked`
    );
    for (var i = 0; i < checkboxes.length; i++) {
      array.push(checkboxes[i].value);
    }
    array.length > 0
      ? searchParams.set(name, array.join("_"))
    ...
    return (
      <div className="mb-1">
        <label className="cstm-chkbx" htmlFor={data.slug}>
          ...
          <input
            type={
              name === "categories" || name === "discounts"
                ? "radio"
                : "checkbox"
            }
            id={data.slug}
            value={data.slug}
            name={name}
            className={`${name}-product-check-input product-check-input`}
            onChange={handler}
