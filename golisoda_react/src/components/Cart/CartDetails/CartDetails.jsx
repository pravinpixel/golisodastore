...
const CartDetails = ({
  ...
  const validateProcess = (data) => {
    ...
    if (
      data.shipping_method.type === "STANDARD_SHIPPING" &&
      data.shipping_method.charge_id === null
    ) {
    ...
    if (
      data.shipping_method.type === "PICKUP_FROM_STORE" &&
      store_address === null
    ) {
  ...
  const codSubmit = () => {
    ...
    const checkData = {
      ...
      shipping_method: {
        ...
        address_id:
          shippingMethod.toUpperCase() === "PICKUP_FROM_STORE"
            ? store_address?.id
            : shipping_address?.customer_address_id,
        charge_id:
          shippingMethod.toUpperCase() === "PICKUP_FROM_STORE"
    ...
    return (
      <>
        ...
        {cartData?.is_cod === 1 && authUser.isLoggedIn && (
          <ul className="list-group mb-3">
            <label
              htmlFor={"cashon_delivery"}
              onChange={(e) => cashOnDeliveryCheck(e)}
              className="list-group-item list-group-item-action d-flex justify-content-between"
            >
              <span>
                <input
        ...
        <div className="mb-2">
          <b className="fw-500 text-primary">Cart details</b>
        </div>
        <div>
            ...
            {coupon ? (
              <li className="list-group-item d-flex align-items-center justify-content-between">
                <span>
                  Coupon{" "}
                  <b>
                    {checkoutData.coupon_type === "percentage"
                      ? `(${Number(checkoutData?.coupon_percentage)}%)`
                      : null}
                  </b>
                </span>
