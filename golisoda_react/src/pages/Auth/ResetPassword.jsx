...
function ResetPassword() {
  ...
  return (
    <div className="section section-center-center">
      {isVerfying === true ? (
        <h2>
          Verifying Your Account <Spinner animation="grow" className="ms-3" />
        </h2>
      ) : (
...
const ResetPasswordComponent = ({customerId}) => {
  const formSchema = Yup.object().shape({
    password: Yup.string()
      ...
      .matches(
        /^(?=.*[a-z])(?=.*[0-9])(?=.{6,})/,
        "Your password Must Contain 6 alpha numeric characters"
      ),
    ...
    <form onSubmit={handleSubmit(FormHandler)} className="col-lg-3">
      ...
      <div className="text-center">
        ...
        <div>
          ...
          <Link to="/" className="btn-link btn btn-sm text-info px-0 border-0">
